...
def apply_rule(rule: dict, content: str, lines: list[str], filepath: str) -> list[Violation]:
    ...
    # Future: module_missing_param, duplicate_handler_names, etc.
    return []
...
ANSIBLE_IGNORE_DIRS = {".git", ".tox", "__pycache__", "node_modules", ".venv", "venv"}

def is_ansible_file(path: Path) -> bool:
    return path.suffix in ANSIBLE_EXTENSIONS

def gather_files(target: str) -> list[Path]:
    p = Path(target)
    if p.is_file():
        return [p] if is_ansible_file(p) else []
    files = []
    for root, dirs, fnames in os.walk(p):
        dirs[:] = [d for d in dirs if d not in ANSIBLE_IGNORE_DIRS]
        for fname in fnames:
            fp = Path(root) / fname
            if is_ansible_file(fp):
                files.append(fp)
    return sorted(files)
...
def generate_html_report(results: list[FileResult], rules: list[dict], output_path: str, target: str):
...
<title>Ansible Code Review Report</title>
...
.logo-badge {
  ...
  background: linear-gradient(135deg, #1e6fd4 0%, #3b8beb 100%);
  display: flex; align-items: center; justify-content: center;
  font-size: 15px; flex-shrink: 0;
}
.logo-title { font-size: 14px; font-weight: 700; letter-spacing: 0.5px; color: var(--text); }
...
/* Grade ring */
.grade-wrap {
  display: flex; align-items: center; gap: 16px; padding: 4px 0 8px;
}
.grade-ring {
  width: 64px; height: 64px; border-radius: 50%; flex-shrink: 0;
  display: flex; align-items: center; justify-content: center;
  font-size: 30px; font-weight: 800; color: var(--grade-col);
  border: 3px solid var(--grade-col);
  box-shadow: 0 0 20px color-mix(in srgb, var(--grade-col) 30%, transparent);
}
.grade-info { flex: 1; }
.grade-score { font-size: 24px; font-weight: 700; color: var(--grade-col); line-height: 1; }
...
.grade-bar-fill {
  ...
  background: linear-gradient(90deg, var(--c-crit), var(--c-warn), var(--c-clean));
  width: var(--score-pct);
...
.file-list-count {
  ...
  background: var(--surface3); color: var(--muted2); flex-shrink: 0;
}
...
.filter-chip {
  ...
  transition: all .15s; letter-spacing: 0.3px; font-family: var(--mono);
}
...
.issue-table thead th .sort-icon { opacity: 0.4; margin-left: 4px; }
...
.file-ref .line-badge {
  ...
  padding: 0 5px; font-size: 10px; margin-left: 4px; color: var(--c-warn);
}
...
.offending-line .ln {
  color: var(--muted); user-select: none; flex-shrink: 0; padding-top: 1px;
  border-right: 1px solid var(--border2); padding-right: 10px; min-width: 32px;
  text-align: right;
}
.offending-line code {
  color: var(--c-warn); flex: 1; white-space: pre-wrap; word-break: break-all;
}
.code-body {
  background: #080a0e; padding: 12px 14px;
}
.code-body pre {
...
/* Offending file line block */
.offending-wrap { grid-column: 1 / -1; }
.offending-label {
...
.file-score-ring {
  ...
  border: 2px solid var(--ring-col); color: var(--ring-col);
  box-shadow: 0 0 12px color-mix(in srgb, var(--ring-col) 25%, transparent);
}
...
.file-sev-pill {
  ...
  border-radius: 3px; font-family: var(--mono); letter-spacing: 0.5px;
  border: 1px solid var(--p-border); color: var(--p-color); background: var(--p-bg);
}
...
<div class="topbar">
  <div class="topbar-left">
    ...
    <div>
      <div class="logo-title">Ansible Code Reviewer</div>
      <div class="logo-sub">Infrastructure Automation · Quality Gate</div>
    </div>
  </div>
...
<div class="layout">
  ...
  <aside class="sidebar">
    ...
    <div class="sidebar-section">
      ...
      <div class="stat-row">
        <div class="stat-pill" style="--pill-color:var(--text)" onclick="setFilter('all')">
          <div class="pill-val" id="stat-total">0</div>
          <div class="pill-name">All Issues</div>
        </div>
        <div class="stat-pill" style="--pill-color:var(--c-clean)" onclick="filterFiles()">
          <div class="pill-val" id="stat-clean" style="color:var(--c-clean)">0</div>
          <div class="pill-name">Clean Files</div>
        </div>
        <div class="stat-pill" style="--pill-color:var(--c-crit)" onclick="setFilter('critical')">
          <div class="pill-val" id="stat-crit" style="color:var(--c-crit)">0</div>
          <div class="pill-name">Critical</div>
        </div>
        <div class="stat-pill" style="--pill-color:var(--c-err)" onclick="setFilter('error')">
          <div class="pill-val" id="stat-err" style="color:var(--c-err)">0</div>
          <div class="pill-name">Error</div>
        </div>
        <div class="stat-pill" style="--pill-color:var(--c-warn)" onclick="setFilter('warning')">
          <div class="pill-val" id="stat-warn" style="color:var(--c-warn)">0</div>
          <div class="pill-name">Warning</div>
        </div>
        <div class="stat-pill" style="--pill-color:var(--c-info)" onclick="setFilter('info')">
          <div class="pill-val" id="stat-info" style="color:var(--c-info)">0</div>
          <div class="pill-name">Info</div>
        </div>
      </div>
    </div>
  ...
  <main class="main">
    ...
    <div id="view-issues">
      <div class="filter-bar" id="filter-bar">
        ...
        <div class="filter-chip" style="--chip-col:var(--c-crit)" onclick="setFilter('critical')" data-sev="critical">
          <span class="chip-dot"></span>Critical
        </div>
        <div class="filter-chip" style="--chip-col:var(--c-err)" onclick="setFilter('error')" data-sev="error">
          <span class="chip-dot"></span>Error
        </div>
        <div class="filter-chip" style="--chip-col:var(--c-warn)" onclick="setFilter('warning')" data-sev="warning">
          <span class="chip-dot"></span>Warning
        </div>
        <div class="filter-chip" style="--chip-col:var(--c-info)" onclick="setFilter('info')" data-sev="info">
          <span class="chip-dot"></span>Info
        </div>
        <input class="filter-search" id="search-box" placeholder="Search rule, file, description…" oninput="applySearch()">
      </div>
    ...
    <div id="view-files" style="display:none">
      <div class="section-hdr">
        ...
        <div class="section-count" id="file-count-label"></div>
      </div>
    ...
    <div id="view-rules" style="display:none">
      <div class="section-hdr">
        <div class="section-title">Rule Reference</div>
        <div class="section-count" id="rules-count-label"></div>
      </div>
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);overflow:hidden;">
        <table class="rules-table">
          <thead><tr>
            <th>ID</th><th>Severity</th><th>Category</th><th>Rule Name</th>
          </tr></thead>
    ...
    <div class="footer">
      Generated by <strong>Ansible Code Reviewer</strong> &mdash; Infrastructure Automation Team
    </div>
  </main>
</div>
...
let currentSort     = { key: 'severity', dir: 1 };
let searchQuery     = '';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('grade-bar').style.setProperty('--score-pct', D.avg_score + '%');
...
function buildSidebar() {
  ...
  container.innerHTML = REPORT_DATA.files.map((f, i) => {
    const col = f.score >= 80 ? 'var(--c-clean)' : f.score >= 60 ? 'var(--c-warn)' : 'var(--c-crit)';
    const cnt = f.violations.length;
    return \`<div class="file-list-item" style="--dot-col:\${col}" id="sfl-\${i}" onclick="focusFile(\${i})">
      ...
      \${cnt ? \`<div class="file-list-count">\${cnt}</div>\` : ''}
    </div>\`;
  }).join('');
}
...
function focusFile(idx) {
  ...
  // Show that file's issues in table
...
function renderIssueTable() {
  ...
  const countLabel = document.getElementById('issue-count-label');

  countLabel.textContent = viols.length + ' issue' + (viols.length !== 1 ? 's' : '');
  ...
  tbody.innerHTML = viols.map((v, idx) => {
    ...
    const shortFile = v._fname;

    return \`
    <tr class="issue-row" id="irow-\${idx}" onclick="toggleDetail(\${idx})">
      <td>
        <div class="sev-tag" style="--tag-color:\${sc};--tag-bg:\${sb};--tag-border:\${sbo}">
          <span class="sev-dot"></span>\${v.severity.toUpperCase()}
        </div>
      </td>
      <td><span class="rule-id-tag">\${esc(v.rule_id)}</span></td>
      <td>
        <div class="issue-name">\${esc(v.rule_name)}</div>
        <div class="issue-cat">\${esc(v.category)}</div>
      </td>
      <td>
        <div class="file-ref" title="\${esc(v._file)}">\${esc(shortFile)}<span class="line-badge">L\${lineDisp}</span></div>
      </td>
...
function buildDetailHTML(v) {
  ...
  return \`<div class="detail-inner">
    ...
    <div class="detail-section" style="grid-column:1/-1">
      <div class="detail-title">Offending Line · \${esc(v._fname || v._file)} · Line \${lineDisp}</div>
      <div class="offending-line">
        <span class="ln">\${lineDisp}</span>
        <code>\${lineContent}</code>
      </div>
    </div>\` : ''}
    \${hasCode ? \`
    <div class="code-wrap">
      <div class="detail-title" style="margin-bottom:10px">Code Fix Guide</div>
      <div class="code-grid">
        \${badCode ? \`
...
function buildFilesView() {
  ...
  container.innerHTML = REPORT_DATA.files.map((f, fi) => {
    ...
    const rows = f.violations.map((v, vi) => {
      const sc = SEV_COLORS[v.severity] || 'var(--muted)';
      const sb = SEV_BG[v.severity];
      const sbo= SEV_BORDER[v.severity];
      const lineDisp = v.line > 0 ? v.line : 'EOF';
      const detailId = \`fv-\${fi}-\${vi}\`;
      return \`
      <tr class="issue-row" id="frow-\${detailId}" onclick="toggleFDetail('\${detailId}')">
      ...
      <tr class="detail-row" id="fdetail-\${detailId}">
        <td colspan="5">\${buildDetailHTML({...v, _fname: f.name, _file: f.path})}</td>
      </tr>\`;
    }).join('');

    const cleanMsg = f.violations.length === 0
      ? '<div class="empty-state" style="padding:30px"><div class="empty-icon" style="font-size:28px">✓</div>No violations — clean file</div>'
      : '';
    ...
    <div class="file-card-v2" id="fcard-\${fi}">
      <div class="file-card-header" onclick="toggleFileCard(\${fi})">
        ...
        <div style="flex-shrink:0;color:var(--muted);font-size:11px;font-family:var(--mono);margin-right:8px">
          \${f.violations.length} issue\${f.violations.length !== 1 ? 's' : ''}
        </div>
        <div class="file-chevron">▾</div>
      </div>
      <div class="file-body-v2">
        \${f.violations.length ? \`<table class="issue-table" style="margin:0"><thead><tr>
          <th>Severity</th><th>Rule ID</th><th>Issue</th><th>Line</th><th></th>
        </tr></thead><tbody>\${rows}</tbody></table>\` : cleanMsg}
      </div>
    </div>\`;
  }).join('');
}
...
function buildRulesTable() {
  ...
  document.getElementById('rules-tbody').innerHTML = REPORT_DATA.rules.map(r => {
    ...
    const sb = SEV_BG[r.severity];
    const sbo= SEV_BORDER[r.severity];
    return \`<tr>
